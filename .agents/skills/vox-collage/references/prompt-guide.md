<!--
  PORTED VERBATIM from the vox-director skill by Alisa Qian
  Source: https://github.com/Alisa0808/vox-director  (references/prompt-guide.md)
  Licensed MIT — Copyright (c) 2026 Atlas Cloud. See vox-collage/SKILL.md "Attribution & license".

  This is the pure LOOK-layer prompt methodology — reproduced unchanged. It is model-agnostic:
  the collage DNA and the flat-safe motion vocabulary hold regardless of which generator runs.
  In THIS repo, execution routes ONLY through ralphy verbs (see SKILL.md) — where this doc names
  the original's scripts (`keyframes.py`, `style_bakeoff.py`) or its Atlas model ids
  (`nano-banana-2`, `gemini-omni-flash`, `kling-video-o3-pro`), read the SKILL.md model-mapping
  table for the ralphy equivalent (`ralphy generate image|video` on our stack). The prompt
  STRUCTURES below are what you actually use.
-->
