...
1. **Read the inputs first.** Read `notes/README.md` (the note shape), `docs/developing-ralphy.md` (English-only rule, the lint suite), and skim `notes/issues/` + `notes/ideas/` + `roadmap/todo/` so you know what already exists. `dev-tasks` is the sibling skill for the board lifecycle.
...
3. **Collision-check before filing each.** Search `notes/issues/`, `notes/ideas/`, and `roadmap/` for an existing entity that already covers the thought. If one exists, **update it** (or note the overlap) rather than spawning a duplicate. Never let the same thing live in two places.
...
5. **Write each issue** as `notes/issues/NNN-kebab-slug.md`, numbered monotonically (`ls notes/issues/ | tail -1`, increment). Follow the `notes/README.md` shape (Status / Filed / Folder / Context / What / Why it matters / Notes) **plus** a concrete **`## Scope / acceptance`** section listing what "done" means — file paths, gates to pass, acceptance criteria — so a downstream agent can execute without re-deriving scope. Add severity + category when useful. Cross-link related issues by number.
...
- **Numbering is monotonic per folder.** Never reuse or renumber existing issues.
