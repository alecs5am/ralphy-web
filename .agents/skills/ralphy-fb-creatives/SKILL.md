...
description: >-
  ...
  This is a niche SKILL (generalized), not a remix TEMPLATE. For "remix this exact ad but swap X", that is the remix path in docs/skills-vs-templates.md. For a single poster, use /ralphy-poster.
---
...
**DO NOT FIRE** when:
- The brief is for ONE still (poster, drop graphic, hero image) → use [`/ralphy-poster`](../ralphy-poster/SKILL.md) instead.
...
8. **Hand off** to `/ralphy-evaluator` if the user wants a quality gate before delivery.
