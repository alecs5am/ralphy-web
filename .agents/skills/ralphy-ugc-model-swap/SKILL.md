...
description: >-
  ...
  Pairs with the remix path in docs/skills-vs-templates.md: remix swaps any element of a template; this skill is the specialist for swapping the on-camera person. For building a UGC ad from scratch, see /ralphy-ugc-ad.
---
...
**DO NOT FIRE** when:
- The user wants a brand-new ad authored from a product (no source clip to mirror) → `/ralphy-ugc-ad`.
...
6. **Render + eval** → `ralphy render <id>` → `/ralphy-evaluator`.
