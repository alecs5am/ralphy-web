...
- **Restaurant interior tour / ambiance reel.** This template is dish-first; for venue / vibe walkthroughs use `cinematic`.
- **Step-by-step recipe with strong VO instruction.** Possible here, but `product-360` style structured beats may fit better if the recipe is the product.
- **Before / after weight-loss / diet ad.** Use `before-after-product` — that's a different conversion pattern.
- **Packaged food ad where the package IS the hero.** Borderline — if the focus is unboxing / package reveal more than the food inside, look at `ecommerce-ad`.
